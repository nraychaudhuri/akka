...
Note that you may receive an empty ``CurrentClusterState`` if you start the subscription
before the initial join procedure has completed. It is nothing wrong with that. When the node
has been accepted in the cluster you will receive ``MemberUp`` for that node, and other 
nodes.
